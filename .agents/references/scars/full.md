...
- Bound checkpoint patch-cache retention by string size as well as entry count. Zero TTL prevents reuse but can leave expired values in the cache; explicitly evict oversized results and test that a burst of large diffs does not displace retained small entries.

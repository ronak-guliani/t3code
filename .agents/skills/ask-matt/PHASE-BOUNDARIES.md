...
**1. Can you continue in this session?** Continue when the next phase needs this phase as a primary source or the remaining context is sufficient to work reliably. Grilling → implementation is the standard yes: the implementation wants the reasoning verbatim, not a summary of it. If context is becoming unreliable, compact at a phase boundary rather than relying on a model-specific token estimate.

---
name: test-audit
description: "Set a quality bar for tests and audit low-value, implementation-coupled, or duplicative coverage and the test-only production seams it preserves. Use whenever writing, changing, reviewing, or sweeping tests."
---
...
whole subsystem's test surface; before starting one, read
...
Follow repository-specific test and formatting instructions.

1. Run the smallest owner and sibling tests using the repository's supported
   test commands.
...
4. Run the changed-file gate required by repository policy.
5. Inspect `git diff --numstat`; report production/tooling separately from
   tests and test support.
6. Run any final review required by repository policy.

## Landing and continuation

Commit, push, or open a PR only when authorized by the user or repository
instructions. Land one coherent change at a time; after landing, refresh from
the target branch and rerun read-only discovery for the next high-confidence
batch.
...
- commit and PR state, when applicable;
- named follow-ups.

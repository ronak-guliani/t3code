...
4. **Grill (if needed).** If the request needs fleshing out, use the installed `grilling` skill one round of questions at a time. The optional `domain-modeling` skill is not installed here, so update `CONTEXT.md` and ADRs directly when decisions sharpen domain terms.

...
- Worktree cleanup intent must be persisted as a durable job from `thread.deleted`; canonicalize paths, reserve pending paths against reassignment, recheck active ownership under the worktree lock, release the lock before Git I/O while the pending row remains the reservation, bound retries for permanent failures, and never force-remove a dirty worktree.

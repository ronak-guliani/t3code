...
- Nested-thread creation must use the authenticated T3 MCP boundary, not ambient provider shell variables: Copilot terminal commands do not inherit `T3_MCP_THREAD_ID`, and a global `t3` binary can target another app flavor. Have the MCP tool inject its authoritative thread ID as `--parent` and route through the active server's CLI path and `baseDir`.

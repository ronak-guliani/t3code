...
export const installPreviewEventForwarding = Effect.fn(
  ...
  const broadcast = yield* PreviewBroadcast.PreviewBroadcast;
  const manager = yield* PreviewManager.PreviewManager;
  yield* manager.subscribeStateChanges((tabId, state) =>
    broadcast.sendAll(IpcChannels.PREVIEW_STATE_CHANGE_CHANNEL, tabId, state),
  );
  yield* manager.subscribeRecordingFrames((frame) =>
    broadcast.sendAll(IpcChannels.PREVIEW_RECORDING_FRAME_CHANNEL, frame),
  );
  yield* manager.subscribePointerEvents((event) =>
    broadcast.sendAll(IpcChannels.PREVIEW_POINTER_EVENT_CHANNEL, event),
  );
});
...
export const importBrowserCookies = DesktopIpc.makeIpcMethod({
  channel: IpcChannels.PREVIEW_IMPORT_BROWSER_COOKIES_CHANNEL,
  ...
  handler: Effect.fn("desktop.ipc.preview.importBrowserCookies")(function* ({
    ...
    return yield* browserImport.importCookies({
      input: { sourceId, sourceProfileDirectory, targetProfileId },
      scope,
      persistent,

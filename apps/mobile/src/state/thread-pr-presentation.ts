...
export function presentThreadPr(
  ...
  return {
    ...
    accessibilityLabel: `#${pr.number} ${presentation.longName} ${isDraft ? "draft" : (pr.state ?? "status unknown")}`,
    textClassName: isDraft
      ? "text-adaptive-zinc-500-400"
      : (PR_STATE_TEXT_CLASS[pr.state] ?? "text-adaptive-zinc-500-400"),
  };
}

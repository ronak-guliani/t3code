...
describe("AgentActivity widget layout", () => {
  ...
  it("deep links the banner to the row that needs attention", () => {
    ...
    expect(JSON.stringify(layout.banner)).toContain(
      '"widgetURL":"t3code-rg://threads/env-1/thread-2"',
    );
  });
  ...
  it("deep links the banner to the first row when nothing needs attention", () => {
    ...
    expect(JSON.stringify(layout.banner)).toContain(
      '"widgetURL":"t3code-rg://threads/env-1/thread-1"',
    );
  });

  it.each(["t3code-rg-dev", "t3code-rg-preview"])("keeps the owned %s scheme", (urlScheme) => {
    const layout = AgentActivity(
      { ...props, urlScheme, activities: [makeRow({})] },
      environment as never,
    );
    expect(JSON.stringify(layout.banner)).toContain(
      `"widgetURL":"${urlScheme}://threads/env-1/thread-1"`,
    );
  });

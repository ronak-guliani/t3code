...
it.effect(
  "recovers a dropped tunnel, preserves installed status, and cleans failed agent activation",
  () =>
    Effect.gen(function* () {
      ...
      yield* Deferred.succeed(exits[0]!, ChildProcessSpawner.ExitCode(255));
      yield* Deferred.await(reconnecting);
      yield* TestClock.adjust("1 second");
      yield* Deferred.await(reconnected);
      expect(forwards).toBe(1);
      expect(modes.filter((mode) => mode === "start")).toHaveLength(3);

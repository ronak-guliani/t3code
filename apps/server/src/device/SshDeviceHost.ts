...
import { runSshCommand, baseSshArgs, resolveSshCommand } from "./sshCommand.ts";
...
export const make = Effect.fn("SshDeviceHost.make")(function* (
  ...
  const environmentId = yield* fs
    ...
    .pipe(
      Effect.catch((error) =>
        Effect.logWarning(
          "Device host environment identity unavailable; using isolated state path",
          { error },
        ).pipe(Effect.as(server.stateDir)),
      ),
    );
  ...
  const stop = lock.withPermit(
    Effect.gen(function* () {
      ...
      connectionScope = null;
      if (activated)
        yield* provide(bootstrap(config, owner, "stop")).pipe(
          Effect.catch((error) =>
            Effect.logWarning("Could not stop remote device helpers", {
              hostId: config.id,
              error: error.message,
            }),
          ),
        ...
        return yield* connect().pipe(
          Effect.onError(() =>
            Effect.gen(function* () {
              ...
              if (failedScope) yield* Scope.close(failedScope, Exit.void);
              if (enabled)
                yield* provide(bootstrap(config, owner, "stop-agent")).pipe(
                  Effect.catch((error) =>
                    Effect.logWarning("Could not clean failed remote agent startup", {
                      hostId: config.id,
                      error: error.message,
                    }),
                  ),
  ...
  const host = {
    ...
    stopAgent: changeAgent(false).pipe(
      Effect.asVoid,
      Effect.catch((error) =>
        onStatus("failed", error.message).pipe(
          Effect.andThen(
            Effect.logWarning("Could not stop remote agent tools", {
              hostId: config.id,
              error: error.message,
            }),
          ),

...
projectionSnapshotLayer("ProjectionSnapshotQuery", (it) => {
  ...
  it.effect("serves concurrent snapshot reads alongside writes", () =>
    Effect.gen(function* () {
      const snapshotQuery = yield* ProjectionSnapshotQuery;
      const sql = yield* SqlClient.SqlClient;
      const threadId = ThreadId.make("thread-concurrent");

      // Full snapshots decode every row they read, and earlier tests leave
      // deliberately undecodable fixtures behind (bad resume_cursor_json,
      // null checkpoint_ref) — clear all snapshot-read tables so this test
      // reads only its own fixtures.
      yield* sql`DELETE FROM projection_turns`;
      yield* sql`DELETE FROM projection_thread_messages`;
      yield* sql`DELETE FROM projection_thread_proposed_plans`;
      ...
      yield* sql`
        INSERT INTO projection_projects (
          ...
          'project-concurrent', 'Concurrent project', '/tmp/concurrent',
          '{"provider":"copilot","model":"gpt-5.4"}', '[]',
          '2026-09-01T00:00:00.000Z', '2026-09-01T00:00:00.000Z', NULL
        )
      `;
      yield* sql`
        INSERT INTO projection_threads (
          ...
          'thread-concurrent', 'project-concurrent', 'Concurrent thread',
          '{"provider":"copilot","model":"gpt-5.4"}', 'approval-required', 'default',
          NULL, 0, 0, 0,
          '2026-09-01T00:00:00.000Z', '2026-09-01T00:00:00.000Z', NULL
        )
      ...
      // Read-only snapshots must not hold a write-lock transaction: a
      // concurrent write has to commit while the 12-query/10-query/9-query/5-query
      // fan-outs are in flight, and every read still resolves consistently.
      const results = yield* Effect.all(
        {
          snapshot: snapshotQuery.getSnapshot(),
          shell: snapshotQuery.getShellSnapshot(),
          detail: snapshotQuery.getThreadDetailById(threadId),
          context: snapshotQuery.getThreadShellProjectContextById(threadId),
          write: sql`
            INSERT INTO projection_projects (
              project_id, title, workspace_root, default_model_selection_json, scripts_json,
              created_at, updated_at, deleted_at
            ) VALUES (
              'project-concurrent-write', 'Concurrent write', '/tmp/concurrent-write',
              '{"provider":"copilot","model":"gpt-5.4"}', '[]',
      ...
      assert.isTrue(results.snapshot.threads.some((thread) => thread.id === threadId));
      assert.isTrue(results.shell.threads.some((thread) => thread.id === threadId));
      assert.equal(results.detail._tag, "Some");
      assert.equal(results.context._tag, "Some");

      const reread = yield* snapshotQuery.getSnapshot();
      assert.isTrue(reread.projects.some((project) => project.id === "project-concurrent-write"));
    }),
  );
});

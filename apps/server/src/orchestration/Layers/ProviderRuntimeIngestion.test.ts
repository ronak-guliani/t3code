...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("settles the active turn when the provider aborts it", async () => {
    ...
    harness.emit({
      type: "content.delta",
      eventId: asEventId("evt-turn-aborted-delta-1"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId,
    ...
    harness.emit({
      type: "content.delta",
      eventId: asEventId("evt-turn-aborted-delta-2"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId,
      ...
      (entry) =>
        ...
        entry.messages.some(
          (message: ProviderRuntimeTestMessage) =>
            message.id === "assistant:item-turn-aborted" &&
  ...
  it("ignores an abort for a stale turn while a newer turn is active", async () => {
    const harness = await createHarness();
    const activeTurnId = asTurnId("turn-current");

    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-turn-current-started"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId: activeTurnId,
    ...
    harness.emit({
      type: "turn.aborted",
      eventId: asEventId("evt-turn-stale-aborted"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
  ...
  it("does not turn a user-requested interruption into a provider error", async () => {
    const harness = await createHarness({ serverSettings: { enableAssistantStreaming: true } });
    const activeTurnId = asTurnId("turn-user-interrupted");
    const now = new Date().toISOString();

    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-turn-user-interrupted-started"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: now,
      turnId: activeTurnId,
    ...
    harness.emit({
      type: "content.delta",
      eventId: asEventId("evt-turn-user-interrupted-delta"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: now,
      turnId: activeTurnId,
      itemId: asItemId("item-turn-user-interrupted"),
      payload: {
        streamKind: "assistant_text",
        delta: "response before interruption",
      },
    });
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        commandId: CommandId.make("cmd-turn-user-interrupted"),
        threadId: asThreadId("thread-1"),
        turnId: activeTurnId,
        createdAt: now,
      }),
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-turn-user-interrupted-settled"),
        threadId: asThreadId("thread-1"),
        session: {
          threadId: asThreadId("thread-1"),
          status: "ready",
    ...
    harness.emit({
      type: "turn.aborted",
      eventId: asEventId("evt-turn-user-interrupted-aborted"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId: activeTurnId,
    ...
    const thread = await waitForThread(
      ...
      (entry) =>
        ...
        entry.messages.some(
          (message: ProviderRuntimeTestMessage) =>
            message.id === "assistant:item-turn-user-interrupted" &&
            message.text === "response before interruption" &&
            !message.streaming,
        ),
    );
  ...
  it("finalizes an older completed turn without disturbing the newer active turn", async () => {
    const harness = await createHarness({ serverSettings: { enableAssistantStreaming: true } });
    const olderTurnId = asTurnId("turn-completed-after-newer-started");
    const activeTurnId = asTurnId("turn-newer-active");

    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-turn-older-started"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId: olderTurnId,
    ...
    harness.emit({
      type: "content.delta",
      eventId: asEventId("evt-turn-older-delta"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId: olderTurnId,
      itemId: asItemId("item-turn-older"),
      payload: {
        streamKind: "assistant_text",
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-turn-newer-started"),
        threadId: asThreadId("thread-1"),
        session: {
          threadId: asThreadId("thread-1"),
          status: "running",
    ...
    harness.emit({
      type: "turn.completed",
      eventId: asEventId("evt-turn-older-completed"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId: olderTurnId,
    ...
    const thread = await waitForThread(
      ...
      (entry) =>
        ...
        entry.messages.some(
          (message: ProviderRuntimeTestMessage) =>
            message.id === "assistant:item-turn-older" &&
            message.text === "older response" &&
            !message.streaming,
        ),
    );
    expect(thread.latestTurn).toMatchObject({
  ...
  it("ignores a late abort after the active turn has already settled", async () => {
    const harness = await createHarness();
    const turnId = asTurnId("turn-already-settled");
    const completionEventId = asEventId("evt-turn-already-settled-completed");
    const completionProcessed = runtime!.runPromise(
      harness.ingestion.awaitTurnCompletionProcessed(completionEventId),
    );

    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-turn-already-settled-started"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId,
    ...
    await completionProcessed;

    harness.emit({
      type: "turn.aborted",
      eventId: asEventId("evt-turn-already-settled-aborted"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId,
  ...
  it("ignores an aborted turn completion after a newer turn becomes active", async () => {
    const harness = await createHarness();
    const abortedTurnId = asTurnId("turn-aborted-before-newer");
    const newerTurnId = asTurnId("turn-newer-after-abort");

    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-turn-aborted-before-newer-started"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
    ...
    harness.emit({
      type: "turn.aborted",
      eventId: asEventId("evt-turn-aborted-before-newer-aborted"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId: abortedTurnId,
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-turn-newer-after-abort"),
        threadId: asThreadId("thread-1"),
        session: {
          threadId: asThreadId("thread-1"),
          status: "running",
    ...
    harness.emit({
      type: "turn.completed",
      eventId: asEventId("evt-turn-aborted-before-newer-completed"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId: abortedTurnId,
    ...
    expect(
      thread?.activities.some(
        (activity: ProviderRuntimeTestActivity) =>
          activity.kind === "insights.turn.completed" && activity.turnId === abortedTurnId,
      ),
    ).toBe(false);
  });
  ...
  it("applies an abort without a provider turn id to the active turn", async () => {
    ...
    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-turn-aborted-without-id-started"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId: activeTurnId,
    ...
    harness.emit({
      type: "turn.aborted",
      eventId: asEventId("evt-turn-aborted-without-id"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      payload: {
        reason: "Provider aborted the turn",
      },
    });
    ...
    const thread = await waitForThread(
      ...
      (entry) =>
        entry.session?.status === "interrupted" &&
        entry.session.activeTurnId === null &&
        entry.latestTurn?.turnId === activeTurnId &&
  ...
  it("clears projected tool update fingerprints when a turn aborts", async () => {
    const harness = await createHarness();
    const turnId = asTurnId("turn-tool-update-aborted");
    const itemId = asItemId("item-tool-update-aborted");
    const toolUpdate = {
      ...
      data: {
        toolCallId: "tool-update-aborted",
        kind: "execute",
        command: "pnpm test",
      },
    };
    ...
    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-tool-update-aborted-started"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId,
    ...
    harness.emit({
      type: "item.updated",
      eventId: asEventId("evt-tool-update-aborted-first"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId,
    ...
    harness.emit({
      type: "turn.aborted",
      eventId: asEventId("evt-tool-update-aborted"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId,
    ...
    harness.emit({
      type: "item.updated",
      eventId: asEventId("evt-tool-update-aborted-second"),
      provider: ProviderDriverKind.make("opencode"),
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId,
    ...
    expect(
      thread?.activities.filter(
        (activity: ProviderRuntimeTestActivity) =>
          activity.kind === "tool.updated" && activity.turnId === turnId,
      ),
    ).toHaveLength(2);
  });
  ...
  it("dispatches coalesced streaming deltas once they exceed the size threshold", async () => {
    const harness = await createHarness({
      ...
      recordAssistantDeltaDispatches: true,
    });
    const now = new Date().toISOString();
    ...
    // Freeze time so the 50ms periodic flusher can never fire; only the
    // size-threshold spill may deliver the text.
    vi.useFakeTimers();
    try {
      expect(harness.assistantDeltaDispatchLog).toEqual([]);
      ...
      // Advance well short of the periodic window while letting the spill
      // dispatch run; the text must arrive through the threshold branch alone.
      let delivered = false;
      for (let elapsed = 0; elapsed < 40 && !delivered; elapsed += 5) {
        await vi.advanceTimersByTimeAsync(5);
        delivered = harness.assistantDeltaDispatchLog.length > 0;
      }
      expect(delivered).toBe(true);
      expect(harness.assistantDeltaDispatchLog).toEqual([oversizedDelta]);
    } finally {
      vi.useRealTimers();
    }
  });
  ...
  it("flushes sub-threshold buffered deltas to the engine when the runtime shuts down", async () => {
    const harness = await createHarness({
      serverSettings: { enableAssistantStreaming: true },
      recordAssistantDeltaDispatches: true,
    });
    const now = new Date().toISOString();
    const tailDelta = " buffered tail";
    ...
    await waitForThread(
      harness.engine,
      (thread) => thread.session?.activeTurnId === "turn-shutdown-flush",
    );

    // First spill delivers a partial chunk, then a sub-threshold delta is
    // left buffered with no terminal event and no periodic window elapsing.
    vi.useFakeTimers();
    try {
      harness.emit({
        type: "content.delta",
        eventId: asEventId("evt-message-delta-shutdown-flush-partial"),
        provider: ProviderDriverKind.make("codex"),
        createdAt: now,
        threadId: asThreadId("thread-1"),
        turnId: asTurnId("turn-shutdown-flush"),
        itemId: asItemId("item-shutdown-flush"),
        payload: { streamKind: "assistant_text", delta: "y".repeat(3_000) },
      });
      for (let elapsed = 0; elapsed < 20; elapsed += 5) {
        await vi.advanceTimersByTimeAsync(5);
      }
      expect(harness.assistantDeltaDispatchLog).toEqual(["y".repeat(3_000)]);

      harness.emit({
        type: "content.delta",
        eventId: asEventId("evt-message-delta-shutdown-flush-tail"),
        provider: ProviderDriverKind.make("codex"),
        createdAt: now,
        threadId: asThreadId("thread-1"),
        turnId: asTurnId("turn-shutdown-flush"),
        itemId: asItemId("item-shutdown-flush"),
        payload: { streamKind: "assistant_text", delta: tailDelta },
      });
      // Keep total elapsed time below the 50ms periodic window so the text is
      // provably still buffered.
      await vi.advanceTimersByTimeAsync(10);
      expect(harness.assistantDeltaDispatchLog).toEqual(["y".repeat(3_000)]);

      // Disposal needs real timers for the Effect runtime's async
      // continuations. The shutdown finalizer begins flushing immediately
      // during dispose, well before the next periodic window could fire, so
      // this exercises exactly the shutdown-vs-periodic-flusher race: the
      // buffered tail must reach the engine exactly once, unlost.
      vi.useRealTimers();
      if (runtime === null) {
        throw new Error("runtime was not initialized");
      }
      await runtime.dispose();
      expect(harness.assistantDeltaDispatchLog).toEqual(["y".repeat(3_000), tailDelta]);
    } finally {
      vi.useRealTimers();
    }
  });

  it("holds a terminal event until its prerequisite delta flush succeeds", async () => {
    let failAssistantDeltaDispatch = true;
    const harness = await createHarness({
      serverSettings: { enableAssistantStreaming: true },
    ...
    const messageId = "assistant:item-terminal-ordering";

    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-turn-started-terminal-ordering"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId,
    ...
    harness.emit({
      type: "content.delta",
      eventId: asEventId("evt-message-delta-terminal-ordering"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId,
      itemId: asItemId("item-terminal-ordering"),
      payload: { streamKind: "assistant_text", delta: "tail before completion" },
    });
    harness.emit({
      type: "item.completed",
      eventId: asEventId("evt-message-completed-terminal-ordering"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId,
    ...
    // While the delta dispatch keeps failing, the completion must not be
    // processed: no finalized message may appear while text is undelivered.
    await new Promise((resolve) => setTimeout(resolve, 120));
    const blockedReadModel = await Effect.runPromise(harness.engine.getReadModel());
    const blockedThread = blockedReadModel.threads.find(
      (entry) => entry.id === asThreadId("thread-1"),
    );
    expect(
      blockedThread?.messages.every((message: ProviderRuntimeTestMessage) => message.streaming),
    ).toBe(true);
    ...
    await waitForThread(harness.engine, (entry) =>
      entry.messages.some(
        (message: ProviderRuntimeTestMessage) =>
          ...
          !message.streaming &&
          message.text === "tail before completion",
      ),
    );

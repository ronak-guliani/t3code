import { OrchestrationCheckpointFile, TrimmedNonEmptyString } from "@t3tools/contracts";
    ...
    execute: (row) =>
      sql`
        ...
        VALUES (
          ...
          ${row.checkpointSpeculativePatch}
        )
    ...
    execute: ({ threadId }) =>
      sql`
        SELECT
          ...
          checkpoint_speculative_patch AS "checkpointSpeculativePatch"
        FROM projection_turns
        WHERE thread_id = ${threadId}
        ORDER BY
          CASE

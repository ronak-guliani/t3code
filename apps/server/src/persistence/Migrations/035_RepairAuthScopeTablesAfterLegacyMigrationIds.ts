...
export default Effect.gen(function* () {
  ...
  const pairingLinksNeedRepair =
    !hasColumn(pairingLinkColumns, "scopes") ||
    !hasColumn(pairingLinkColumns, "proof_key_thumbprint") ||
    hasColumn(pairingLinkColumns, "role");
  const sessionsNeedRepair =
    !hasColumn(sessionColumns, "scopes") ||
    !hasColumn(sessionColumns, "client_device_type") ||
    !hasColumn(sessionColumns, "last_connected_at") ||
    hasColumn(sessionColumns, "role");

  if (!pairingLinksNeedRepair && !sessionsNeedRepair) {
    return;
  }

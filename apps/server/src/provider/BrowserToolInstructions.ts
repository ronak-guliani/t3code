...
- Treat \`open-browser\`, \`reconnect-required\`, \`configure-target\`, \`resolve-environment-mismatch\`, and \`use-supported-browser\` as actionable blocked states; do not retry pairing blindly or conclude that pairing failed from a generic navigation error.
- If preflight reports no attached tab, call \`preview_preflight({open:true})\` or \`preview_open\`; a closed tab is different from an unsupported browser. Complete this recovery before concluding that the browser is unavailable. If an MCP tool returns \`invalid_mcp_credential\` with \`reconnect-required\`, restart the chat/session before retrying.

...
- Use \`create_isolated_workspace\` for this thread's new checkout and \`switch_workspace\` for an existing one; delegate child checkouts through each child's \`workspace\` field, never a handoff tool.
...
export function buildCopilotMcpServerOptions(
  ...
  toolsetNames.add("delegate_work");

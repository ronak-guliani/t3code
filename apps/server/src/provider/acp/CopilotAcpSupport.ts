...
- MCP tools may be deferred instead of appearing in the initially loaded tool list. When a requested \`t3-code\` or \`t3-tools\` tool is deferred, you MUST use the tool-search API to load that exact function definition, then call it. For browser work, search \`t3-code\` for the needed \`preview_*\` function. For a new workspace for the current thread, search \`t3-tools\` for \`create_isolated_workspace\`; for one delegated child, search for \`create_nested_thread\`; for multiple sibling children, search for \`create_nested_threads\`. Do not use an MCP resources/list result as an availability check: zero non-invokable resources does not mean the server exposes zero tools. Never report a deferred tool missing based only on the initially loaded tools or resources.

...
import { PullRequestProviderKind } from "@t3tools/contracts";
import type {
  PullRequestAction,
  PullRequestActor,
...
export interface ProviderChangeRequest {
  ...
  /** Accounts with a review requested. */
  readonly reviewRequestLogins: ReadonlyArray<string>;
  /**
   * The provider reported an outstanding team review request. The service uses this only when
   * the host itself selected the row for the viewer's Reviewing query.
   */
...
export interface PullRequestProviderApi {
   ...
   *
   * One call for both directions: GitHub posts and deletes the same collection. Asking again
   * somebody who has already reviewed is a request like any other — which is how a re-request is
   * made.
   */
  ...
  /**
   * Fresh monitoring snapshot for durable server-owned PR monitors.

 ...
 * `first` over 100 with EXCESSIVE_PAGINATION and REST clamps `per_page` to it. Asking for 100
 * here would request 101 and buy a whole second round trip for one row (measured: `gh pr list
 * --limit 100` makes 1 HTTP request, `--limit 101` makes 2).
 */
...
function providerDetail(error: PullRequestProviderError): string {
  ...
  return error.reason === "missing-tool"
    ? "GitHub CLI (`gh`) is required to browse pull requests on this host. Install it from https://cli.github.com/ and reload."
...
/**
 * The GitHub repository identity. `displayName` is the path below the host; owner/name is the
...
export const make = Effect.gen(function* () {
    ...
    projections.getShellSnapshot().pipe(
      ...
      Effect.map((snapshot) => {
        ...
        for (const project of snapshot.projects) {
          ...
          const host = pullRequestHostOf(project.repositoryIdentity);
  ...
  const toEntry = (input: {
    ...
    /** The provider's Reviewing query, not a broad listing, selected this row. */
    readonly reviewingQuerySelected: boolean;
    ...
    return {
      ...
      viewerReviewRequested:
        ...
        (input.item.reviewRequestLogins.some((login) => login.toLowerCase() === viewer) ||
          (input.reviewingQuerySelected && input.item.hasTeamReviewRequest)),
  ...
  const listUncached: PullRequestService["Service"]["list"] = (input) =>
    Effect.gen(function* () {
      ...
      const { supported: projects, viewerRoots } = yield* listWorkspaceProjects(input);
  ...
  const runAction: PullRequestService["Service"]["runAction"] = (input) =>
    requireProject(input).pipe(
      Effect.flatMap((project): Effect.Effect<void, PullRequestError> => {
        ...
        // A strategy GitHub does not offer must be refused rather than silently defaulted.
        if (
  ...
  const statsCacheKey = (ref: PullRequestRef) =>
    JSON.stringify([listingsEpoch, refEpoch(ref), ref.projectId, ref.repository, ref.number]);
  const recentStats = new Map<
    string,
    { readonly at: number; readonly value: PullRequestDiffStat }
  >();
  const recordStats = (ref: PullRequestRef, value: PullRequestDiffStat, at: number) => {
    const key = statsCacheKey(ref);
  ...
  const detailCodec = Schema.fromJsonString(PullRequestDetailSchema);
  const detailCache = yield* Cache.makeWith(
    (key: string) => {
      ...
      return persistedRead(reference, "detail", detailCodec, detailUncached(reference)).pipe(
        Effect.tap(
  ...
  // Exact batches share in-flight reads; overlapping pages reuse each row already fetched by a
  // detail read. Explicit refreshes and mutations strand both forms through their epochs.
  const listStats: PullRequestService["Service"]["listStats"] = Effect.fn(
    ...
    for (const stat of result.stats) {
      ...
      if (ref !== undefined) recordStats(ref, stat, at);
    }

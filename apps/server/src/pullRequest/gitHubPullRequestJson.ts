...
function toListItem(raw: Schema.Schema.Type<typeof RawListItemSchema>): GitHubPullRequestListItem {
  return {
    ...
    headBranch: raw.headRefName.trim(),
    baseBranch: raw.baseRefName.trim(),
    state: toState(raw),
    ...
    items.push({
      ...toListItem({
        ...
        reviewRequests: (node.reviewRequests?.nodes ?? []).flatMap(
          (request): ReadonlyArray<Schema.Schema.Type<typeof RawReviewRequestSchema>> => {
            const login = trimmed(request?.requestedReviewer?.login);
            const slug = trimmed(request?.requestedReviewer?.slug);
            return login !== null ? [{ login }] : slug !== null ? [{ slug }] : [];
          },
        ),

...
export function jitterMs(baseMs: number, ratio = 0.2): number {
  const spread = Math.floor(baseMs * ratio);
  if (spread <= 0) return baseMs;
  return baseMs + Math.floor(Math.random() * (spread * 2 + 1)) - spread;
}
...
export function nextPollDelayMs(input: {
  ...
  if (input.readiness?.ready) {
    return jitterMs(POLL_READY_MS);
  }
  if (input.hadActionableEvents || (input.readiness?.blockers.length ?? 0) > 0) {
    return jitterMs(POLL_ACTIVE_MS);
  }
  return jitterMs(POLL_BASE_MS);
}

...
} from "../orchestration/Services/ValidationCoordinatorReactor.ts";
...
import { selectFocusedTestFiles } from "./ValidationPolicy.ts";
import { isBrowserGateKind, isRepositoryGateKind } from "./ValidationGateExecutor.ts";
...
export const isValidationEvent = (event: OrchestrationEvent): boolean =>
...
export interface ValidationLifecycleShape {
  readonly request: (
  ...
  readonly reconcileAll: () => Effect.Effect<void, never>;
  readonly nextLeaseSweepDelay: (maxIntervalMs: number) => Effect.Effect<number>;
}

export class ValidationLifecycle extends Context.Service<
  ValidationLifecycle,
  ValidationLifecycleShape
>()("t3/validation/ValidationLifecycle") {}

export const makeValidationLifecycle = Effect.gen(function* () {
  const orchestrationEngine = yield* OrchestrationEngineService;
  ...
  const planRequest = Effect.fn("ValidationLifecycle.planRequest")(function* (
  ...
  const releaseLease = Effect.fn("ValidationLifecycle.releaseLease")(function* (
  ...
  const interruptActiveGates = Effect.fn("ValidationLifecycle.interruptActiveGates")(function* (
  ...
  const interruptExpiredGate = Effect.fn("ValidationLifecycle.interruptExpiredGate")(function* (
    thread: import("@t3tools/contracts").OrchestrationThread,
    run: NonNullable<import("@t3tools/contracts").OrchestrationThread["validationRun"]>,
    gate: NonNullable<
      NonNullable<
    ...
    yield* orchestrationEngine.dispatch({
      ...
      blockerReason: null,
      diagnostics: [...gate.diagnostics, "Validation lease expired while the gate was executing."],
  ...
  const reconcileRun = Effect.fn("ValidationLifecycle.reconcileRun")(function* (runId: string) {
    const readModel = yield* orchestrationEngine.getReadModel();
  ...
  const reconcileRequest = Effect.fn("ValidationLifecycle.reconcileRequest")(function* (
    request: ValidationRequest,
  ) {
  ...
  const reconcileAll = Effect.fn("ValidationLifecycle.reconcileAll")(function* () {
    if (reconciling) {
      queued = true;
      return;
    }
  ...
  const request: ValidationLifecycleShape["request"] = (input: ValidationCoordinatorRequest) =>
    Effect.gen(function* () {
  ...
  return {
    ...
    reconcileAll: () => reconcileSafely(reconcileAll()),
    nextLeaseSweepDelay: (maxIntervalMs) =>
  ...
  } satisfies ValidationLifecycleShape;
});
...
export const ValidationLifecycleLive = Layer.effect(ValidationLifecycle, makeValidationLifecycle);

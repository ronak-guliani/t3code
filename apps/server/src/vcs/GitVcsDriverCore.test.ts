...
it.layer(TestLayer)("GitVcsDriver core integration", (it) => {
  ...
  describe("review diff previews", () => {
    ...
    it.effect("returns source-level errors instead of hiding failed branch diffs", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        ...
        assert.equal(workingTree?.status, "ready");
        assert.equal(branchRange?.status, "error");
        assert.include(branchRange?.error ?? "", "Failed to load branch diff");
        assert.equal(branchRange?.metadata.filesChanged, 0);
      }),
    );

    it.effect("includes server-computed file safety metadata", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        yield* initRepoWithCommit(cwd);
        const driver = yield* GitVcsDriver.GitVcsDriver;
        yield* writeTextFile(cwd, "src/app.ts", `export const value = "${"\u202E"}";\n`);

        const preview = yield* driver.getReviewDiffPreview({ cwd });
        const workingTree = preview.sources.find((source) => source.kind === "working-tree");

        assert.equal(workingTree?.status, "ready");
        assert.equal(workingTree?.metadata.filesChanged, 1);
        assert.equal(workingTree?.metadata.largeFiles, 0);
        assert.equal(workingTree?.files[0]?.path, "src/app.ts");
        assert.equal(workingTree?.files[0]?.hasHiddenBidiChars, true);
      }),
    );
  });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          assert.equal(
            yield* git(remote, ["--git-dir", remote, "log", "-1", "--pretty=%s", "main"]),
            "Add upstream update",
          );
    ...
    it.effect("pushes to the requested remote instead of the primary remote", () =>
      Effect.gen(function* () {
        ...
        const originMain = yield* driver.execute({
          ...
          cwd: originRemote,
          args: ["--git-dir", originRemote, "show-ref", "--verify", "--quiet", "refs/heads/main"],
          allowNonZeroExit: true,
          timeoutMs: 10_000,
        });

...
import { analyzeReviewDiff, emptyReviewDiffAnalysis } from "../review/DiffPreviewMetadata.ts";
...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const getReviewDiffPreview = Effect.fn("getReviewDiffPreview")(function* (
    ...
    const readySource = Effect.fn("GitVcsDriver.getReviewDiffPreview.readySource")(function* (
      source: Omit<ReviewDiffPreviewSource, "status" | "error" | "diffHash" | "files" | "metadata">,
    ) {
      const analysis = analyzeReviewDiff(source.diff);
      return {
        ...source,
        status: "ready" as const,
        error: null,
        diffHash: yield* hashDiff(source.diff),
        files: analysis.files,
        metadata: analysis.metadata,
      };
    });
    const errorSource = Effect.fn("GitVcsDriver.getReviewDiffPreview.errorSource")(function* (
      source: Omit<
        ReviewDiffPreviewSource,
        "status" | "error" | "diffHash" | "files" | "metadata"
      > & { readonly error: string },
    ) {
      const analysis =
        source.diff.length > 0 ? analyzeReviewDiff(source.diff) : emptyReviewDiffAnalysis();
      return {
        ...source,
    ...
    const formatDiffError = (label: string, error: GitCommandError): string =>
      `${label}: ${error.detail}`;

    const dirtyTrackedResult = yield* Effect.match(
      executeGit(
    ...
    const dirtyUntracked = yield* Effect.match(readUntrackedReviewDiffs(input.cwd), {
      onFailure: (error) => ({ _tag: "Left" as const, left: error }),
      onSuccess: (value) => ({ _tag: "Right" as const, right: value }),
    });
    const dirtyDiff = [
      dirtyTrackedResult._tag === "Right" ? dirtyTrackedResult.right.stdout.trimEnd() : "",
      dirtyUntracked._tag === "Right" ? dirtyUntracked.right.diff.trimEnd() : "",
    ]
      .filter((diff) => diff.length > 0)
      .join("\n");
    const dirtyErrors = [
      dirtyTrackedResult._tag === "Left"
        ? formatDiffError("Failed to load tracked working tree diff", dirtyTrackedResult.left)
        : null,
      dirtyUntracked._tag === "Left"
        ? formatDiffError("Failed to load untracked working tree diff", dirtyUntracked.left)
        : null,
    ].filter((error): error is string => error !== null);
    const dirtyTrackedTruncated =
      dirtyTrackedResult._tag === "Right" && dirtyTrackedResult.right.stdoutTruncated;
    const dirtyUntrackedTruncated =
      dirtyUntracked._tag === "Right" && dirtyUntracked.right.truncated;
    ...
    const baseResult =
      baseRef && branch
        ...
        : null;
    const baseDiff = baseResult?._tag === "Right" ? baseResult.right.stdout : "";
    const baseTruncated = baseResult?._tag === "Right" ? baseResult.right.stdoutTruncated : false;

    const sources: ReviewDiffPreviewSource[] = [
      yield* dirtyErrors.length > 0
        ? errorSource({
            id: "working-tree",
            kind: "working-tree",
            ...
            headRef: null,
            diff: dirtyDiff,
            truncated: dirtyTrackedTruncated || dirtyUntrackedTruncated,
            error: dirtyErrors.join(" "),
          })
        : readySource({
            id: "working-tree",
            kind: "working-tree",
      ...
      yield* baseResult?._tag === "Left"
        ? errorSource({
            ...
            diff: baseDiff,
            truncated: false,
            error: formatDiffError("Failed to load branch diff", baseResult.left),
          })
        : readySource({
            id: "branch-range",
            kind: "branch-range",

...
export default function DiffPanel({ mode = "inline", composerDraftTarget }: DiffPanelProps) {
  ...
  const selectedSourceError =
    !selectedTurn && selectedGitSource?.status === "error" ? selectedGitSource.error : null;
  const selectedSourceMetadata = !selectedTurn ? selectedGitSource?.metadata : undefined;
  const isLoadingSelectedPatch = selectedTurn
    ? activeCheckpointDiff.isPending
    : branchDiffPreview.isPending;
  const selectedPatchError = selectedTurn
    ? activeCheckpointDiff.error
    : (selectedSourceError ?? branchDiffPreview.error);
  const selectedSourceSafetyMessage =
    selectedSourceMetadata &&
    (selectedSourceMetadata.largeFiles > 0 || selectedSourceMetadata.unrenderableFiles > 0)
      ? [
          selectedSourceMetadata.largeFiles > 0
            ? `${selectedSourceMetadata.largeFiles} large file${selectedSourceMetadata.largeFiles === 1 ? "" : "s"}`
            : null,
          selectedSourceMetadata.unrenderableFiles > 0
            ? `${selectedSourceMetadata.unrenderableFiles} unrenderable file${selectedSourceMetadata.unrenderableFiles === 1 ? "" : "s"}`
            : null,
        ]
      ...
      {!activeThread ? (
        ...
        <>
          <div className="diff-panel-viewport flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
            ...
            {selectedSourceSafetyMessage && (
              <p className="shrink-0 border-b border-border/70 bg-muted/40 px-3 py-1.5 text-[11px] text-muted-foreground">
                Diff contains {selectedSourceSafetyMessage}; rendering may be limited.
              </p>
            )}
            {selectedSourceError && renderablePatch && (
              <p className="shrink-0 border-b border-red-500/20 bg-red-500/5 px-3 py-1.5 text-[11px] text-red-500/80">
                {selectedSourceError}
              </p>
            )}

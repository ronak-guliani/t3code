...
import {
  ...
  createBuildConfig,
  DESKTOP_ASAR_UNPACK,
  hasKnownStaleSchemaDefectReference,
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("detects stale desktop bundles that reference Schema.Defect as a function value", () => {
    assert.isTrue(
      hasKnownStaleSchemaDefectReference("cause: effect.Schema.optional(effect.Schema.Defect)"),
    );
    assert.isTrue(hasKnownStaleSchemaDefectReference("cause: Schema.optional(Schema.Defect)"));
    assert.isFalse(
      hasKnownStaleSchemaDefectReference("cause: effect.Schema.optional(effect.Schema.Defect())"),
    );
  });
  ...
  it.effect("adds passkey entitlements and the alpha protocol to signed macOS builds", () =>
    Effect.gen(function* () {
  ...
  it.effect("uses separate identifiers and artifact names for dev builds", () =>
    Effect.gen(function* () {
      const config = yield* createBuildConfig(
        "mac",
        "dmg",
  ...
  it("keeps production and dev URL scheme ownership separate", () => {
    assert.deepStrictEqual(resolveDesktopProtocolSchemes("alpha"), ["t3code"]);
    assert.deepStrictEqual(resolveDesktopProtocolSchemes("dev"), ["t3code-dev"]);
  });

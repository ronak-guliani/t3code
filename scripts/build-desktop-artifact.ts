...
export class MissingDesktopBuildInputError extends Schema.TaggedErrorClass<MissingDesktopBuildInputError>()(
  "MissingDesktopBuildInputError",
  {
    ...
    buildCommand: Schema.Literal("vp run --filter @t3tools/desktop build"),
  },
) {
  override get message(): string {
...
export class InvalidServerBundleError extends Schema.TaggedErrorClass<InvalidServerBundleError>()(
  "InvalidServerBundleError",
  {
  ...
  override get message(): string {
    return `Server bundle failed to import: ${this.bundlePath}`;
  }
}

export class InvalidDesktopBundleError extends Schema.TaggedErrorClass<InvalidDesktopBundleError>()(
  "InvalidDesktopBundleError",
  {
    bundlePath: Schema.String,
    reason: Schema.Literal("stale-schema-defect-reference"),
  },
) {
  override get message(): string {
    return `Desktop bundle is stale or invalid (${this.reason}): ${this.bundlePath}`;
  }
}
...
const resolveWorkspaceBinaryCommand = Effect.fn("resolveWorkspaceBinaryCommand")(function* (
  repoRoot: string,
  binaryName: string,
  ...
  const command = path.join(
    ...
    "node_modules",
    ".bin",
    hostPlatform === "win32" ? `${binaryName}.cmd` : binaryName,
  );
  return yield* resolveSpawnCommand(command, args, options);
});

const validateServerBundleImport = Effect.fn("validateServerBundleImport")(function* (
  bundlePath: string,
) {
  yield* Effect.tryPromise({
    try: () => import(NodeUrl.pathToFileURL(bundlePath).href),
    catch: (cause) => new InvalidServerBundleError({ bundlePath, cause }),
  });
});

export function hasKnownStaleSchemaDefectReference(contents: string): boolean {
  return (
    contents.includes("Schema.optional(Schema.Defect)") ||
    contents.includes("Schema.optional(effect.Schema.Defect)")
  );
}

const validateDesktopBundle = Effect.fn("validateDesktopBundle")(function* (bundlePath: string) {
  const fs = yield* FileSystem.FileSystem;
  const contents = yield* fs.readFileString(bundlePath);
  if (hasKnownStaleSchemaDefectReference(contents)) {
    return yield* new InvalidDesktopBundleError({
      bundlePath,
      reason: "stale-schema-defect-reference",
    });
  }
});
...
export function resolveDesktopAppId(flavor: typeof DesktopBuildFlavor.Type): string {
  return flavor === "dev" ? "com.t3tools.t3code.dev" : DESKTOP_APP_ID;
}
...
export function resolveDesktopProtocolSchemes(
  flavor: typeof DesktopBuildFlavor.Type,
): ReadonlyArray<string> {
  return flavor === "dev" ? ["t3code-dev"] : ["t3code"];
}
  ...
  if (platform === "mac") {
    buildConfig.mac = {
      ...
      protocols: [
        {
          name: "T3 Code",
          schemes: resolveDesktopProtocolSchemes(flavor),
        },
      ],
      ...(macPasskeySigning
        ? {
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  if (!options.skipBuild) {
    ...
    const spawnCommand = yield* resolveWorkspaceBinaryCommand(repoRoot, "vp", [
      "run",
      "--filter",
      "@t3tools/desktop",
      "build",
    ...
    yield* runCommand(
      ...
      { label: "vp run --filter @t3tools/desktop build", verbose: options.verbose },
    );
  }
  ...
  for (const input of requiredBuildInputs) {
    if (!(yield* fs.exists(input.artifactPath))) {
      return yield* new MissingDesktopBuildInputError({
        ...input,
        buildCommand: "vp run --filter @t3tools/desktop build",
      });
    }
  }
  ...
  yield* validateDesktopBundle(path.join(distDirs.desktopDist, "main.cjs"));
  yield* validateServerBundleImport(path.join(distDirs.serverDist, "bin.mjs"));
  ...
  const installCommand = yield* resolveWorkspaceBinaryCommand(repoRoot, "vp", [
    ...STAGE_INSTALL_ARGS,
  ]);
  yield* runCommand(
  ...
  const builderCommand = yield* resolveWorkspaceBinaryCommand(repoRoot, "vp", builderArgs, {
    env: buildEnv,
  });
  yield* runCommand(
